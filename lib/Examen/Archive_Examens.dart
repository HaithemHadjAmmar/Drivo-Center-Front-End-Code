...
class Archive_examnes extends StatefulWidget {
  const Archive_examnes({Key? key}) : super(key: key);

  @override
  State<Archive_examnes> createState() => _Archive_examnesState();
}

class _Archive_examnesState extends State<Archive_examnes> {
  ...
  Future<List<Examen>> rechercheExamen(String query) async {
    try {
      final response = await http.get(
          Uri.parse('http://10.0.2.2:8000/api/recherche/examens?query=$query'));
  ...
  Future<List<Examen>> getExamens() async {
    ...
    final response = await http.get(Uri.parse('http://10.0.2.2:8000/api/candidats/exams/${user?.id}'));
    if (response.statusCode == 200) {
      print(user?.id);
      print(response.body);
  ...
  Widget build(BuildContext context) {
    return Scaffold(
        appBar: AppBar(
          title: Text('Historique Examens', style: TextStyle(
